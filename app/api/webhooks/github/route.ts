...
async function summarizeWithOpenAI(commits: any[], repoName: string): Promise<string> {
  ...
  const prompt = `You're writing a short, casual "project update" post for a developer's personal portfolio site (a running log readers can follow — think a devlog entry, not a press release).
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!process.env.OPENAI_API_KEY) {
      console.error('OPENAI_API_KEY is not set — cannot summarize commit.');
      return NextResponse.json({ error: 'OpenAI is not configured.' }, { status: 500 });
    }

    const summary = await summarizeWithOpenAI(commits, matchedProject.name);
